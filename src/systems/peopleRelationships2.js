...
export function recordSupportEvent(peopleStateRaw,relationshipsRaw,event={}){
  const state=ensurePeople2State(peopleStateRaw),relationships=normalizeRelationships(relationshipsRaw);const actorId=String(event.actorId??'unknown');const sourceId=String(event.sourceId??`${event.kind??'help'}:${actorId}:${event.seasonNumber??1}:${event.week??0}`);const existing=state.supportHistory.find(x=>x.sourceId===sourceId);if(existing)return{state,relationships,event:existing,duplicate:true};
  ...
  state.supportHistory.push(row);state.seenSourceIds.push(sourceId);
  if(relationships[actorId]){let rec=relationships[actorId];rec.lifecycle.supportIds.push(row.id);rec.lifecycle.sacrifice.money+=row.money;rec.lifecycle.sacrifice.time+=row.time;rec.lifecycle.sacrifice.labor+=row.labor;const magnitude=Math.min(5,Math.max(1,Math.round((row.money/250)+(row.time/4)+(row.labor/4)+(row.nonCashValue/300))));rec=applyRelationshipChange(rec,{trust:magnitude*.4,closeness:magnitude*.5,reliability:magnitude*.5,availability:-Math.max(0,magnitude-3)*.2},{id:`rel-support:${sourceId}`,seasonNumber:row.seasonNumber,week:row.week,reason:`support:${row.kind}`,source:sourceId});relationships[actorId]=rec;}
  return{state,relationships,event:row,duplicate:false};
}

export function supportSummary(peopleStateRaw,{seasonNumber=null}={}){
  const s=ensurePeople2State(peopleStateRaw);const rows=seasonNumber==null?s.supportHistory:s.supportHistory.filter(x=>x.seasonNumber===Number(seasonNumber));const byPerson={};
