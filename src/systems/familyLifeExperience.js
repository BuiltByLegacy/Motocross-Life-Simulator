import { createFamilyLifeBuilderState, startingCircumstances, familyBuilderSummary } from './familyLifeBuilder.js';
...
const LEGACY_MAP=Object.freeze({
  ...
  rich:{financial:'wealthy',motocrossKnowledge:'weekend',household:'two_parent',school:'private',supportModel:'professional',home:'workshop'},
  homeschool:{financial:'comfortable',motocrossKnowledge:'weekend',household:'two_parent',school:'homeschool',supportModel:'family_diy',home:'basic'},
  ...
  clueless:{financial:'comfortable',motocrossKnowledge:'new',household:'two_parent',school:'public',supportModel:'family_diy',home:'basic'},
});
...
export function ensureFamilyLifeSelection(raw={}){return {version:2,builder:createFamilyLifeBuilderState(raw.builder??raw.familyLifeBuilder??{}),supportHome:createRacingSupportHomeState(raw.supportHome??raw.racingSupportHome??{})};}
...
export function initializeFamilyLife(game,builderRaw={},supportRaw={}){
  ...
  game.state.familyLife={version:2,initialized:true,builder,supportHome,story};game.state.schoolMode=schoolMode(builder.school);game.family.money=start.economy.startingCash;
  game.state.people2??={version:2,supportEvents:[],seenSupportSourceIds:[]};game.state.relationships=game.state.relationships??{};
  for(const person of uniqById(support.people))game.state.relationships[person.id]=game.state.relationships[person.id]??person;
  game.state.familyLife.school={id:builder.school,...start.school};game.state.familyLife.garage={...home.garage};game.state.familyLife.equipment={...home.equipment};game.state.familyLife.property={...home.property};game.state.familyLife.compoundSeed={...home.compoundSeed};game.state.familyLife.raceSupport={...support};game.state.familyLife.opportunityContext={...start.opportunityContext};game.state.familyLife.riderPerformanceModifiers={};
  game.state.careerEconomy=undefined;ensurePeopleEconomyState(game);return{ok:true,state:game.state.familyLife};
}

export function migrateFamilyLifeSave(saveRaw){
  if(!saveRaw?.state)return saveRaw;const save=structuredCloneSafe(saveRaw);if(save.state.familyLife?.version>=2)return save;
  const migrated=migrateLegacyFamilySelection(save.state.background),story=buildFamilyStory(migrated.builder,migrated.supportHome);
  save.state.familyLife={version:2,initialized:true,migratedFromLegacy:true,builder:migrated.builder,supportHome:migrated.supportHome,story,riderPerformanceModifiers:{}};save.state.schoolMode=save.state.schoolMode??schoolMode(migrated.builder.school);return save;
}
