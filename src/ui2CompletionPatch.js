...
function renderCareer(app) {
  ...
  const resultRows = results.length
    ? results.slice(-5).reverse().map((r) => ui2El('div', { class: 'ui2-paper-row' },
      ui2El('span', {}, r.race ?? `Week ${r.week}`),
      ui2El('strong', {}, r.missed ? 'DNS' : r.dnf ? 'DNF' : `P${r.overall}`)))
    : [ui2El('p', { class: 'ui2-muted' }, 'The first results have not been written yet.')];
  const historyRows = history.length
    ? history.map((h) => ui2El('div', { class: 'ui2-timeline-entry' },
      ui2El('div', { class: 'ui2-timeline-year' }, String(h.year ?? 'Season')),
      ui2El('div', {}, ui2El('strong', {}, `${h.klass ?? 'Class'} · ${h.points ?? 0} pts`),
        ui2El('small', {}, `Best ${h.bestFinish ? `P${h.bestFinish}` : '—'} · ${h.wins ?? 0}W`))))
    : [ui2El('p', { class: 'ui2-muted' }, 'Your first season is still being written.')];
  const memoryRows = memories.length
    ? memories.map((m) => ui2El('article', { class: 'ui2-memory-slip' },
      ui2El('strong', {}, m.title), ui2El('small', {}, m.summary ?? '')))
    : [ui2El('p', { class: 'ui2-muted' }, 'No defining memories yet.')];
  ...
  return ui2El('div', { class: 'ui2-recordbook', 'data-testid': 'ui2-career' },
    ui2El('header', { class: 'ui2-recordbook-hero' },
      ...
      ui2El('p', {}, lifecycle.chosenPosture ? `This season: ${lifecycle.chosenPosture.replaceAll('-', ' ')}` : 'A career is built one season at a time.'),
    ...
    section('This season', [
      ui2El('div', { class: 'ui2-paper-list' }, ...resultRows),
      ui2El('button', { class: 'ui2-text-action', type: 'button', onclick: () => go(app, 'journal') }, 'Open results & memories →'),
    ], 'ui2-career-season'),
    section('Support & opportunity', [
      ui2El('div', { class: 'ui2-support-note' },
        ui2El('div', {},
        ...
        ui2El('button', { class: 'ui2-secondary-action', type: 'button', 'data-testid': 'ui2-career-sponsors', onclick: () => go(app, 'sponsors') }, 'Sponsor Desk')),
    ], 'ui2-career-support'),
    section('Career timeline', [ui2El('div', { class: 'ui2-timeline' }, ...historyRows)], 'ui2-career-history'),
    section('What will be remembered', [ui2El('div', { class: 'ui2-memory-strip' }, ...memoryRows)]));
}

function renderPeoplePanel(app) {
  const people = app.game.relationships?.all?.() ?? [];
...
function renderWorld(app, originalRenderPhone) {
  ...
  if (panel === 'people') return renderPeoplePanel(app);
  if (panel === 'history') {
    const memories = app.game.memory?.top?.(10) ?? [];
    const rows = memories.length
      ? memories.map((m) => ui2El('article', { class: 'ui2-memory-slip' }, ui2El('strong', {}, m.title), ui2El('small', {}, m.summary ?? '')))
      : [ui2El('p', { class: 'ui2-muted' }, 'The history starts when the stories do.')];
    return ui2El('div', { class: 'ui2-world-panel', 'data-testid': 'ui2-world-history' },
      ui2El('button', { class: 'ui2-backlink', type: 'button', onclick: () => { app._ui2WorldPanel = 'hub'; app.render(); } }, '‹ World'),
      ui2El('div', { class: 'ui2-kicker' }, 'Culture & memory'),
      ui2El('h1', {}, 'The World You Are Leaving Marks On'),
      ui2El('div', { class: 'ui2-memory-strip' }, ...rows));
  }
  ...
  return ui2El('div', { class: 'ui2-world-hub', 'data-testid': 'ui2-world' },
    ui2El('header', { class: 'ui2-world-hero' },
      ui2El('div', { class: 'ui2-kicker' }, String(region).replaceAll('_', ' ')),
      ui2El('h1', {}, 'The Motocross World'),
      ui2El('p', {}, 'People, tracks, shops, media and opportunities all connect here.')),
    ui2El('div', { class: 'ui2-world-grid' },
      ui2El('button', { class: 'ui2-world-destination people', type: 'button', 'data-testid': 'ui2-world-open-people', onclick: () => { app._ui2WorldPanel = 'people'; app.render(); } },
        ui2El('strong', {}, 'People'), ui2El('span', {}, 'Family, rivals, coaches and the relationships behind the racing.')),
      ui2El('button', { class: 'ui2-world-destination market', type: 'button', 'data-testid': 'ui2-world-open-market', disabled: market && !market.accessible, onclick: () => { app._phoneApp = 'marketplace'; app.render(); } },
        ui2El('strong', {}, 'Marketplace'), ui2El('span', {}, market?.accessible === false ? (market.lockReason ?? 'Not available yet') : 'Bikes, parts, deals and the stuff that moves through the paddock.')),
      ui2El('button', { class: 'ui2-world-destination phone', type: 'button', 'data-testid': 'ui2-world-open-phone', onclick: () => { app._phoneApp = social?.accessible === false ? 'notifications' : 'social'; app.render(); } },
        ui2El('strong', {}, 'Phone & Media'), ui2El('span', {}, 'Messages, notifications, social attention and connected riders.')),
      ui2El('button', { class: 'ui2-world-destination history', type: 'button', 'data-testid': 'ui2-world-open-history', onclick: () => { app._ui2WorldPanel = 'history'; app.render(); } },
        ui2El('strong', {}, 'Culture & History'), ui2El('span', {}, 'Memories, race culture and the moments that become part of motocross lore.'))));
}
...
function renderOnboarding(app) {
  ...
  let content;
  if (o.step === 'campaign') {
    content = ui2El('div', { class: 'ui2-start-stage' },
      validSave ? ui2El('button', { class: 'ui2-continue-career', type: 'button', 'data-testid': 'ui2-continue-career', onclick: () => app.continueGame() },
        ui2El('small', {}, 'Continue career'), ui2El('strong', {}, `${save.state.rider.name} · ${save.state.rider.klass ?? 'Motocross life'}`), ui2El('span', {}, 'Back to the garage →')) : null,
    ...
    content = ui2El('div', { class: 'ui2-start-stage' },
      ui2El('button', { class: 'ui2-backlink', type: 'button', onclick: () => setStep('campaign') }, '‹ Back'),
      ui2El('div', { class: 'ui2-kicker' }, 'The rider'), ui2El('h1', {}, 'Put a name on the number plate'),
    ...
    content = ui2El('div', { class: 'ui2-start-stage' },
      ui2El('button', { class: 'ui2-backlink', type: 'button', onclick: () => setStep('identity') }, '‹ Back'),
      ui2El('div', { class: 'ui2-kicker' }, 'Where the story starts'), ui2El('h1', {}, 'Every family begins somewhere'),
    ...
    content = ui2El('div', { class: 'ui2-start-stage' },
      ui2El('button', { class: 'ui2-backlink', type: 'button', onclick: () => setStep('background') }, '‹ Back'),
      ui2El('div', { class: 'ui2-kicker' }, 'How much do you want to manage?'), ui2El('h1', {}, 'Set the pace of the life'),
      ...
      ui2El('button', { class: 'ui2-primary-action', type: 'button', 'data-testid': 'ui2-begin-career', onclick: () => app.startGame({ ...o }) }, `Begin ${year} season →`));
  }
...
export function installUi2CompletionPatch(App) {
  ...
  App.prototype.renderTitle = function renderUi2Onboarding() { return renderOnboarding(this); };
