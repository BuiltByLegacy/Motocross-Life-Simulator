...
test('Sponsorship 2.0 survives preseason, calendar, garage, save/reload and renewal branches', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    app.startGame({ name: 'E2E Rider', depth: 'detailed', birthdate: '2014-05-15', campaign: 'rider', avatar: '🧒', background: null });
  ...
  await page.evaluate(async () => {
    ...
    state.pursuit.responses.push({ sponsorId: 'declined-local', sponsorName: 'Declined Local Shop', seasonYear: app.game.seasonYear, type: 'decline', fitScore: 22, support: { cash: 0, productValue: 0, contingency: 0 } });

    const base = {
      id: 'offer-e2e-counter', sponsorId: 'e2e-graphics-counter', sponsorName: 'Counter Graphics', category: 'graphics', tier: 2,
      seasonYear: app.game.seasonYear, status: 'draft', leverage: 72, guardianRequired: true, guardianApproved: false,
      ...
      obligations: [{ type: 'graphics-placement', label: 'Run Counter graphics', required: true, slot: 'rear-fender' }, { type: 'content', label: 'Post Counter reveal', required: true }],
    ...
    const countered = s2.counterOffer(state, base, { cashRetainer: 425, discountPercent: 15 }, {
      rider: { age: app.game.rider.age, results: 75, reputation: 70, professionalism: 90, visibility: 60 }, relationship: 65, careerSeed: '349-browser-counter',
    });
    state = countered.state;
    state.pursuit.responses.push({ sponsorId: base.sponsorId, sponsorName: base.sponsorName, seasonYear: app.game.seasonYear, type: 'mixed-support', fitScore: 82, support: { cash: 350, productValue: 250, contingency: 100 } });

    const signable = {
      id: 'offer-e2e-sign', sponsorId: 'e2e-graphics', sponsorName: 'E2E Graphics', category: 'graphics', tier: 3,
      seasonYear: app.game.seasonYear, status: 'draft', leverage: 88, guardianRequired: true, guardianApproved: false,
  ...
  expect(revised).toMatchObject({ phase: 'funding', contracts: signed.contracts, offers: signed.offers, tab: 'sponsors' });

  await page.getByRole('button', { name: /Lock season/ }).click();
  expect(await page.evaluate(() => window.__legacy.game.state.sponsorship2.preseason.phase)).toBe('locked');

  await page.evaluate(() => { const app = window.__legacy; app.tab = 'week'; app._seasonView = true; app.render(); });
  ...
  await expect(seasonAgenda.locator('[data-s2-obligation-id]').first()).toContainText('E2E Graphics');

  const pending = seasonAgenda.locator('[data-s2-obligation-id]').filter({ has: page.locator('[data-s2-action="fulfill"]') });
  expect(await pending.count()).toBeGreaterThanOrEqual(2);
  await pending.nth(0).locator('[data-s2-action="fulfill"]').click();
  const refreshedAgenda = page.locator('[data-s2-calendar="agenda"]').filter({ hasText: 'Sponsor commitment agenda' });
  const remaining = refreshedAgenda.locator('[data-s2-obligation-id]').filter({ has: page.locator('[data-s2-action="miss"]') });
  ...
  await page.reload();
  await page.waitForFunction(() => !!window.__legacy);
  const continueButton = page.getByRole('button', { name: /Continue Career/ });
