...
test('People + Economy 2.0 multi-season arc reconciles through canonical rollover and survives reload',async({page})=>{
 ...
 expect(result.expenseOk).toBe(true);
 expect(result.recOk).toBe(true);
 ...
 expect(result.ledgerIds).toEqual(['s1-national-trip','s1-bike-sale','s1-work','s2-supported-trip']);
 ...
 const restored=await page.evaluate(async()=>{const close=await import(new URL('src/systems/peopleEconomyCloseout.js',document.baseURI).href);const g=window.__legacy.game;const audit=close.closeoutAudit({relationships:g.state.relationships,people2:g.state.people2,careerEconomy:g.state.careerEconomy,currentBalance:g.family.money});return{auditOk:audit.ok,seasonNumber:g.state.seasonNumber,careerHistory:g.state.careerHistory.length,role:g.state.relationships.dad.lifecycle.role,supportIds:g.state.people2.supportHistory.map(x=>x.sourceId),ledgerIds:g.state.careerEconomy.ledger.map(x=>x.sourceId),peopleIds:Object.keys(g.state.relationships).sort(),money:g.family.money};});
 expect(restored.auditOk).toBe(true);expect(restored.seasonNumber).toBe(2);expect(restored.careerHistory).toBe(1);expect(restored.role).toBe('Advisor');expect(restored.supportIds.filter(x=>x==='dad-s1-wrench')).toHaveLength(1);expect(restored.ledgerIds.filter(x=>x==='s1-bike-sale')).toHaveLength(1);expect(new Set(restored.peopleIds).size).toBe(restored.peopleIds.length);expect(restored.peopleIds).toEqual(result.peopleIds);expect(restored.money).toBe(result.money);await noOverflow(page);
});
