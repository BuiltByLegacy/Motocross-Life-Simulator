...
test('People + Economy 2.0 multi-season arc reconciles and survives reload',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{
  ...
  integ.payCareerExpense(a.game,1200,{sourceId:'s1-national-trip',category:'race-weekend',description:'National trip',funding:[{source:'sponsor',amount:500,type:'support'}]});
  ...
  integ.payCareerExpense(a.game,800,{sourceId:'s2-supported-trip',category:'race-weekend',description:'Supported regional',funding:[{source:'team',amount:550,type:'team-paid'}]});
  ...
  return{before,after,recOk:rec.ok,auditOk:audit.ok,talentUnchanged:startingTalent===JSON.stringify(a.game.rider.skills),money:a.game.family.money,role:a.game.state.relationships.dad.lifecycle.role,supportCount:a.game.state.people2.supportHistory.length,ledgerCount:a.game.state.careerEconomy.ledger.length};
 });
 expect(result.before.viable).toBe(false);expect(result.after.viable).toBe(true);expect(result.recOk).toBe(true);expect(result.auditOk).toBe(true);expect(result.talentUnchanged).toBe(true);expect(result.role).toBe('Advisor');expect(result.supportCount).toBeGreaterThan(0);expect(result.ledgerCount).toBeGreaterThan(3);
