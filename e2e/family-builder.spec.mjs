...
test('Build Your Family Life replaces archetype picker and survives reload on mobile',async({page})=>{
  ...
  await page.getByTestId('family-guardian-name-0').fill('Casey');
  await page.getByRole('button',{name:'Next ›'}).click();
  ...
  await expect(page.getByTestId('family-story-narrative')).toContainText('Casey');
  ...
  expect(before.parentName).toBe('Casey');
  ...
  expect(after.parentName).toBe('Casey');
